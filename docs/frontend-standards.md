...
| Textarea | `ui-textarea` | `value` (model), `rows`, `placeholder`, `mono` — monospace for content read as columns (the CSV paste box). An input on the component and not a font applied from the page: the inner `textarea`'s own declaration outranks anything the host sets, so a page that tried would silently get the body face |
