...
| Tabs | `ui-tabs` | `tabs: TabDef[]`, `active` (model, required), `panels`; methods `tabDomId(id)` / `panelDomId(id)`. `role="tab"` is a promise of a keyboard model and this keeps it: Left/Right/Home/End move selection **and** focus, with a roving `tabindex` so the strip is one tab stop. **The panel is the caller's**: wrap the revealed block in `role="tabpanel"` with `panelDomId(active())` and `aria-labelledby="tabDomId(active())"`, then set `[panels]="true"` — it defaults to false because an `aria-controls` pointing at a missing id is worse than none. The strip owns its own `overflow-x` with a fading right edge: four pt-BR settings tabs are ~470px of unshrinkable content in a 358px column, and without it the *main region* scrolled sideways with nothing on screen to say a fourth tab existed (the document-level `scrollWidth` check cannot see that) |
