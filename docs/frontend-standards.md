...
- **Two greys, and two accent tiers.** `--muted` is *decoration* and is below
  ...
  count, micro-heading, breadcrumb and placeholder.

  **If it is made of words, it is not `--muted`.** The split above was declared
  and then not followed: an audit found **89** `color: var(--muted)` sites
  against 28 using `--muted-strong`, and almost all of the 89 were words — page
  subtitles, card meta lines, table cells, empty states, tab labels, the
  "collection not found" sentence, the conflict notice's hint, and `ui-field`'s
  own form label at 10px. Every one of them measured 2.4–4.4:1 depending on
  theme, which is why `themes.spec.ts` deliberately does **not** hold `--muted`
  to AA: the exemption only holds while nothing readable uses it, so each of
  those sites was quietly cashing in an exemption it had not earned. The test
  cannot catch this — it checks the palette, not who spends it.

  The surviving legitimate uses are few enough to name, and each one repeats
  something already said beside it: a chevron inside its own labelled control,
  an initial-letter watermark above the name it stands for, the dimmed
  no-photo mark, and the `›` between breadcrumbs. If a new use is not of that
  shape, it is `--muted-strong`. Anything under `--fs-xs` (11px) is also
  suspect for the same reason `mono-label` was raised from 10px: tracking that
  makes small caps read as a heading is also what stops them being read.
...
| Field | `ui-field` | `label` (required), `hint` — the mono uppercase label plus an optional note under the control. **It labels the control for real**: after each render it finds the first `input`/`select`/`textarea` in its projected content, gives it an id if it has none and points the label's `for` at it. It used to render the label as a *sibling* with no `for`, which made it a caption — an audit found 8 unnamed controls on the add-item form alone, every one of them already inside a `ui-field`, across 37 call sites. The search excludes `<button>` deliberately: implicit or first-labelable association would bind the label to `ui-tag-input`'s tag-removal button, which precedes its text field, and would name and activate a delete control. Finding nothing (a `ui-field` wrapping a `ui-toggle`) leaves the label unassociated, which is correct — a switch carries its own `ariaLabel`, and a false association is worse than none. The `hint` stays visually associated only |
| Text input | `ui-text-input` | `value` (model), `placeholder`, `type`, `variant: 'panel' | 'subtle'`, `ariaLabel`, `invalid`, `required`, `describedBy`, `clearable`, `clearLabel`; outputs `keydown`, `blurred`, `cleared`; method `focus()`. A placeholder is **not** an accessible name — pass `ariaLabel` for any field the page labels by proximity. `invalid`/`required`/`describedBy` and `focus()` exist so a form can say "this field is wrong, the sentence under it says why" and move focus there **without reaching into this component's DOM**, which is what the item form had to do before they existed: `[attr.aria-invalid]` written at the usage site lands on the `<ui-text-input>` wrapper, which is not the field. `clearable` renders a clear button inside the field, only while there is text |
| Textarea | `ui-textarea` | `value` (model), `rows`, `placeholder`, `ariaLabel`, `mono` — monospace for content read as columns (the CSV paste box). An input on the component and not a font applied from the page: the inner `textarea`'s own declaration outranks anything the host sets, so a page that tried would silently get the body face. `ariaLabel` for the same reason as `ui-text-input`'s |
...
| Card | `ui-card` | `interactive` (hover/press/focus-within affordance), `dashed` — the panel surface. **`interactive` paints an affordance; it does not create a control.** This is a plain custom element with no role, tabindex or href, so `(click)`/`[routerLink]` on the host is mouse-only — `RouterLink` on a non-anchor registers a click listener and nothing else. Four surfaces shipped unreachable by keyboard that way. Put the real control elsewhere: wrap the card in an `<a>`/`<button>` when it holds nothing else interactive (see `app-group-card`), or when it does — a checkbox, a reorder pair — make the card's *title* the `<a>`/`<button>` and give it `.stretch-hit` from `styles.scss`, which spreads its hit area over the card while leaving the nested controls clickable |
...
| Radio | `ui-radio` | `name` (required, groups the set), `optionValue` (required), `value` (model, the set's selection), `hint`, `disabled`; output `picked(value)`. A real `<input type="radio">`, for the same reason `ui-checkbox` is a real checkbox: only the platform gives the role, the checked state, arrow-key movement within the group and the "3 of 3" announcement. **It never preselects anything** — `value` starts `null` and stays there until a user picks, because a disposition for a destructive act ships with nothing chosen (rule 9) and a preselected option is one an impatient user confirms without reading. Five sites hand-rolled this before it existed, and every one of them is the last question before something irreversible: the CSV duplicate rule, the archive-collision choice, and the group-delete disposition that decides whether a group's items are re-parented, unfiled or destroyed |
...
| Section label | `ui-section-label` | `level: 'h2' | 'h3' | null` (default `null`); content-projected mono uppercase micro-heading. Pass `level` where the label genuinely titles a region and the same paint renders as a real heading. Every page once rendered one `<h1>` and no heading below it — a screen reader's heading list, the primary way a non-visual user skims, was one entry long on every screen. It stays optional because not every call site is a heading: the section-header label lives inside a filter `<button>`, and a heading inside a control is the opposite lie |
