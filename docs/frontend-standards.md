...
- **Structural tokens do not vary by theme.** Spacing (`--sp-1`…`--sp-12`),
  ...
  `--ease-in-out`), layering (`--z-sticky/dropdown/overlay/toast/modal/notice`),
  the scrim (`--scrim`) and focus (`--focus-width`, `--focus-offset`) live in
  their own structural `:root`. A theme changes what the app looks like; it does
  not change how far apart two things sit or which one is on top.
- **The layering scale is the only place a layer is named**, and its order is
  the order of interruption: sticky chrome (10) → dropdown (100) → overlay
  (500) → toast (700) → modal (800) → notice (900). A toast sits *under* a
  dialog: it used to outrank it, and since both dock to the bottom of a phone
  screen a two-line Portuguese message was drawn over a dialog's Cancel and
  Delete buttons. `--z-notice` is the conflict notice, the app's only statement
  that a save did **not** happen, and nothing may cover it. `src/styles/z-index.spec.ts`
  greps every `.ts`/`.scss` under `src/app` and fails on a raw `z-index` above
  2 (2 and below is a local stacking order inside one component); it carries a
  `PENDING` list of files still to migrate, which only ever shrinks.
- **`--scrim` is the one way to dim the page.** It is
  `color-mix(in srgb, var(--bg) 78%, transparent)` — derived from a token rather
  than a literal black, because a scrim has to dim towards the theme's own
  ground or it reads as a hole in a dark theme and as soot in a light one. Two
  overlays had invented two different values.
...
| --- | --- | --- |
| Button | `ui-button` | `variant: 'primary' \| 'ghost' \| 'danger' \| 'link' \| 'icon'`, `size: 'md' \| 'sm'`, `block`, `disabled`, `muted`, `type`, `ariaLabel` (required when the label is a bare glyph; also becomes the tooltip), `ariaExpanded`/`ariaControls`/`controlId` for a button that discloses something — these land on the **inner** `<button>`, because the `<ui-button>` host is neither focusable nor announced, so an attribute placed there is never reached — content-projected label. `link` is a text action inside a dense row, `icon` a bare glyph (the ✕ that removes a copy, a field, a member). `pending` implies `disabled`, sets `aria-busy` and draws a CSS-only spinner before the unchanged label — rule 20 wants a write affordance to stop offering itself while its own write runs, and expressing that as a swapped i18n key gave the app `shell.loadFailed.retrying`, `conflict.reloading` and one caller that simply greyed out and said nothing. Bind it from `VaultStore.saving(id)`, passed down as an input. `muted` reads as unavailable but still fires: `disabled` would be the obvious choice and is the wrong one, because a dead control cannot say *why* it is dead — removing the tenant's owner has an explanation the click is what surfaces. An action that navigates stays a real `<a>`; see the `a.link` note in `collection-settings-page.scss` |
| Field | `ui-field` | `label` (required), `hint`, `error`, `controlId`. **It really labels the control it wraps**: the field mints an id, points its `<label for>` at it and hands it to the nested `ui-*` control through the `UI_FIELD_OWNER` injection token, so nesting *is* the association and no call site has anything to remember. The `hint` and the `error` are wired into the control's `aria-describedby`, and an `error` also sets `aria-invalid` and paints the control's border `--danger`. Before this, all 32 labels in the app were decoration: clicking one did nothing and 29 controls announced as unnamed. Put the message on the field, not in a toast — state the outcome ("Give the item a name before saving"), never the rule. A second control inside one field gets its own generated id rather than a duplicate |
| **Field control** (base) | — | `UiFieldControl` in `shared/ui/field/field-control.ts`. Every form control extends it: it supplies `controlId` / `describedBy` / `invalid` inputs, resolves the id (enclosing field's, then its own, then generated), computes `aria-describedby` and `aria-invalid`, and exposes **`focus(options?)` and `selectAll()`**. Name the real focusable element `#control` in the template. `focus()` is the answer to the app's most common bug: the `autofocus` attribute is honoured only on initial page load and does nothing to content inserted later, so a revealed input must be focused explicitly from an `afterNextRender` or a microtask |
| Text input | `ui-text-input` | `value` (model), `placeholder`, `type`, `variant: 'panel' \| 'subtle'`, `ariaLabel`, `autocomplete`, `name`, `inputMode`, `required`, plus the `UiFieldControl` API above; outputs `keydown`, `blurred`. `autocomplete`/`name` are not decoration: with no name, no id and an unassociated label, no password manager could fill or save the login form. Below `$bp-lg` the box is `--tap` tall |
| Textarea | `ui-textarea` | `value` (model), `rows`, `placeholder`, `mono`, `ariaLabel`, `autocomplete`, `name`, `required`, plus the `UiFieldControl` API — monospace for content read as columns (the CSV paste box). An input on the component and not a font applied from the page: the inner `textarea`'s own declaration outranks anything the host sets, so a page that tried would silently get the body face |
| Select | `ui-select` | `value` (model), `options: SelectOption[]`, `disabled`, `ariaLabel`, `name`, plus the `UiFieldControl` API; add class `compact` for dense rows. The chosen option's full text becomes the control's `title`, because a closed `<select>` clips its value and the group picker's options are indented paths — a narrow column otherwise shows whitespace and a fragment. Touch still has no answer to that; see the `ui-tooltip` gap below |
...
| Toggle | `ui-toggle` | `on` (model), `ariaLabel`, `disabled`, plus the `UiFieldControl` API — rendered as `role="switch"`. Below `$bp-lg` the *target* grows to 44px through a pseudo-element while the 36×20 pill stays put, because a switch stretched to 44px tall stops reading as a switch. A switch turns something on. **Never use it to select a row** — that is `ui-checkbox`, and assistive technology announces the two differently |
| Checkbox | `ui-checkbox` | `checked` (model), `indeterminate`, `disabled`, `ariaLabel`, plus the `UiFieldControl` API; output `picked({ checked, shift })`. A real `<input type="checkbox">`, not a styled button, because only the platform gives you the `indeterminate` dash a tri-state "select all" needs — plus the role, the checked state and space-bar activation for free. `picked` carries the modifier keys so a list can implement shift-click ranges without reading the event; shift+**Space** is the keyboard equivalent, since the platform dispatches a real click carrying `shiftKey`. Below `$bp-lg` it grows a 44px target through a centred pseudo-element rather than by growing the 15px box, which a dense table row depends on Below `$bp-lg` the 44px target is a `<label>` wrapping the input, **not** a pseudo-element on the host: a pseudo-element's hit test resolves to the element that owns it, and the host is not a control and has no handler, so the grown area was hit-testable and completely inert for as long as it existed. The label's own activation behaviour is deliberately not relied on — it synthesises a click that drops the modifier keys, which would have broken shift-click ranges in the grown area while leaving them working on the 15px paint. It carries `data-tap-ok` on the host, and `verify:browser` now *presses* inside that claim rather than measuring it. ADR-87 |
| Tabs | `ui-tabs` | `tabs: TabDef[]`, `active` (model, required); roving `tabindex`, Left/Right/Home/End moving selection *and* focus, and the selected tab scrolled into view. **Pair it with `[uiTabPanel]`** on the block the tabs reveal — `<div [uiTabPanel]="strip">` where `strip` is a template ref to the `ui-tabs`. The directive sets `role`, the `id` and `aria-labelledby` on its own host and *registers* with the strip, so `aria-controls` is emitted only when a panel exists and only on the selected tab. There used to be a `panels` boolean instead, and a boolean cannot work here: it is a promise the caller makes about a relationship it does not own, so every unselected tab pointed `aria-controls` at an id that did not exist — the exact thing this component's docblock forbade. The broken state is now unspellable. ADR-86 |
...
| Icon | `ui-icon` | `name` (required, one of the 27 in `ICON_NAMES`), `size`, `strokeWidth`, optional `label`. Inline Feather-style SVG. With `label` it is `role="img"` + `aria-label`; without, `aria-hidden="true"` — so an icon beside its own text label never double-announces. **A raw Unicode glyph is not an icon**: add a name to `ICON_NAMES` instead. The set was four names, which is exactly why the app had accumulated `⌖ ⚙ ⟩ ▤ ☰ ▲ ✕` as substitutes, at the cost of baseline alignment, cross-platform consistency, screen-reader predictability and two extra webfonts. `icon.spec.ts` asserts every name in `ICON_NAMES` draws geometry, because a mistyped `@case` renders an empty `<svg>` and nothing else complains |
| Reorder | `ui-reorder` | `label` (names the item for screen readers), `first`, `last`, `index`, `count`; output `moved(-1 | 1)` — the keyboard half of a drag-to-reorder list, absolutely positioned over a `position: relative` parent. Pass `index`/`count` and the labels state the position without a move ("Move Zelda later (now 3 of 12)") and each move is announced through an internal `role="status"`; without them a keyboard user pressed the button and heard nothing while the list silently re-ordered. The last move focuses the *sibling* first, because the browser blows focus off a control that becomes `disabled` Takes `index`/`count` so a move announces its new position. A boundary arrow is **`muted`, not `disabled`**, so it stays focusable and says why through `aria-disabled`; `move()` declines there. Focus is re-taken on the pressed arrow *after* the re-order renders — the old code aimed pre-emptively at a sibling, which both moved the user off the control they were operating and aimed at an element the caller's re-render then destroyed, dropping focus on `<body>` anyway |
| Section label | `ui-section-label` | content-projected mono uppercase micro-heading |
| Dropdown | `ui-dropdown` | `width`, public `open` and `panelId`; project trigger via `[ddTrigger]`, panel via `[ddPanel]`; call `close()` from panel handlers. **It has a keyboard model**: Escape closes and returns focus to the trigger, `close()` always does the same (the chosen item is usually about to be destroyed, and focus on a removed element becomes focus on `<body>`), Up/Down/Home/End move between the panel's focusable children, and a letter jumps to the next one starting with it. Opened from the keyboard focus steps into the panel; opened by pointer it does not. Bind the trigger's `[attr.aria-expanded]`/`[attr.aria-controls]` from `open()`/`panelId` — `ui-button` has inputs for both. Deliberately **no** `role="menu"`: one panel in the app opens with a static block of profile text, and a menu whose children are not `menuitem`s announces as empty. The panel's side is measured, so a trigger near the left edge does not hang its panel off-screen |
...
| Toast | `ui-toast` | none — global outlet in the shell, driven by `ToastService.flash(message, action?)`. **Two always-present, always-empty live regions** (`role="status"` polite, `role="alert"`) do the announcing, and the drawn toast carries no role of its own: assistive technology announces changes *inside* a region it was already observing, so the old outlet — which created the region together with its text — announced nothing except errors, `role="alert"` being the one shape screen readers still catch on insertion. **Every** toast can be dismissed, whatever its tone. Information lasts `TOAST_DURATION_MS` (4 s: the corner is not where the user is looking). Tones `info \| success \| error`. **An error is painted with `--danger` and never auto-dismisses** — a failure the user did not see is a failure they think succeeded — while info and success keep the short timeout. Messages **queue** instead of cancelling the previous one (a single slot meant two messages in a row showed one), an error head holds the queue behind it, and identical text already showing is dropped, which is what lets a page's own `catch` and the global interceptor both report without the user reading it twice. Tone is never colour-only: each carries a text marker, and errors are `role="alert"`. `action` renders the undo affordance **A `success` retracts an error that is still on screen**, because a success is evidence the failed thing now works and an error has no timer of its own; an `info` does not, since it says nothing about whether the failure still holds. That makes the tone a caller picks load-bearing: on an autosaving page "group added" is a change to the *draft* and belongs on `flash`, or a local confirmation retracts a save failure that is still true. `ToastService` registers with `SessionReset`, since an error otherwise outlives its session and names a collection the next person cannot see |
| Dialog | `ui-dialog` | `title` (required), `describedBy`, `role: 'dialog' \| 'alertdialog'`; output `dismissed`; default slot plus a projected `[dlgActions]`. Owns the scrim, `aria-modal`, Escape, and initial focus — on the **panel**, deliberately not on a control, so a held Enter cannot answer a dialog before it has been read. It also **traps Tab inside the panel** (which is the whole of what `aria-modal` promises and cannot deliver), **restores focus to whatever opened it** on close, **locks the page's scroll** while it is up (outermost dialog only, so a confirmation closing does not unlock a page still covered), and listens for Escape on the *document* so it answers even before focus reaches it — with a stack, so one Escape closes one dialog. The scrim is `var(--scrim)`, derived from the theme's own ground. **`dismissed` always means "nothing happened"**: Escape and the scrim both fire it, and a caller must never read either as a confirmation. A dialog you cannot leave by reflex is one people answer at random. `size: 'default' \| 'wide'` — `wide` (780px) is for a dialog whose body is a table rather than a sentence, and changes nothing below `560px`, where the panel is already docked full-width. The body **pays for the focus ring it would otherwise clip**: a scrolling body asks for `overflow-y: auto`, which makes `overflow-x` compute to `auto` too, so the ring a full-width control draws outside itself was cut down both sides while staying whole top and bottom. It reserves `--focus-width + 2 × --focus-offset` as inline padding and spends it back as negative margin, so nothing moves. Pinned by `verify:browser` |
...
| Focus return | *(helper)* | `returnFocus(opener, document)` + `MAIN_LANDMARK_ID` in `shared/ui/focus-return.ts`. One landing spot for focus after a control destroys itself by succeeding: prefer the opener, fall through to `#main-content`. `<body>` is explicitly not an opener. Used by `ui-inline-edit` (Enter and Escape, *after* the terminal act — moving focus fires `blur`, and an Escape that blurred would otherwise commit), by `ui-dialog` on destroy, and by `LayoutService` after a navigation **only when focus was actually lost**, so a reader stepping through items with `←`/`→` is not thrown to the top each time |
...
| `core/utils/csv.util.ts` | delimited text and nothing above it: `detectDelimiter` (counts `;`/`,`/tab **outside quotes** in the header), `parseCsv` (RFC-4180 quoting, CRLF, and it strips the BOM Excel writes — left in place it becomes part of the first heading and the Name column vanishes with nothing on screen to explain why), `toCsv`. Every record carries the **line it started on**, not the one it ended on, so an error points at the right place in the user's file |
| `core/utils/money.util.ts` | `ceilToCents`, `SUPPORTED_CURRENCIES`, and `parseAmount` — the inverse of `formatMoney` and as tolerant as that one is strict, because its inputs are typed fields, spreadsheet cells and columns copied out of the app's own table. **The last separator decides:** whichever of `.` and `,` comes last is the decimal point when one or two digits follow, every separator is a thousands mark otherwise. That reads `4.200,00`, `4,200.00`, `4.200` and `12,5` without being told which locale wrote them — no fixed rule can, since `1,234` is a thousand in en-US and one-and-a-bit in pt-BR. A currency symbol, a space and the `—` the table prints for "nothing" all pass, and anything with no digit is `0`, which is exactly what `Item.value` means by "not estimated" |
| `core/utils/currency.util.ts`, `date.util.ts`, `focal.util.ts`, `download.util.ts` | as before |
...
`bulk-patch.ts` (`applyBulkPatch`, `removeItems`), `column-prefs.ts`,
